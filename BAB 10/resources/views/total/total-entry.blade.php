...
<div class="form-container-entol">
    <h2>Entry Total</h2>
        <form action="{{ url('/totals/store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <input class="input" type="text" name="jabatan" id="jabatan" placeholder="Jabatan Pegawai" value="{{ old('jabatan') }}" />
        @error('jabatan')
        <p style="font-size: 10px; color: red">{{ $message }}</p>
        @enderror
